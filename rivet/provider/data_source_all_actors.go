...
func (r *ActorsResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
    ...
    tags_json := new(string)
	if !data.Tags_json.IsUnknown() && !data.Tags_json.IsNull() {
		*tags_json = data.Tags_json.ValueString()
	} else {
		tags_json = nil
	}
    
    include_destroyed := new(boolean)
	if !data.Include_destroyed.IsUnknown() && !data.Include_destroyed.IsNull() {
		*include_destroyed = data.Include_destroyed.ValueBoolean()
	} else {
		include_destroyed = nil
	}
	...
	request := actors.ActorsListRequest{
		Project: project,
	...
	res, err := r.client.List(ctx, request)
	if err != nil {
